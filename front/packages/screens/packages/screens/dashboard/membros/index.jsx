...
const MainContent = () => {
    ...
    const [membroEmailToDelete, setMembroEmailToDelete] = useState(null);
    ...
    const handleEditMembroPress = (membro) => {
        setIsModalVisible(true);
        setIsEditMode(true);
    ...
    const renderNoMembers = () => (<NoItemsFound message="Nenhum membro encontrado."/>);
    const renderMembers = () => (<Grid _extra={{ className: "gap-5" }}>
      {membros.map((item, index) => (<GridItem _extra={{
          ...
          <VStack space="md" className="border border-border-300 rounded-lg p-4">
            <HStack space="xl" className="items-center justify-between">
              ...
              <HStack space="md">
                <Pressable onPress={() => item.Usuario?.email !== undefined &&
    ...
    return (<Box className="flex-1">
      <VStack className="p-4 pb-0 md:px-10 md:pt-6 w-full" space="2xl">
        <VStack space="lg" className="items-center">
          <Button className="gap-3 relative" onPress={handleCadastrarMembroPress}>
            <ButtonText>Cadastrar Membro</ButtonText>
          </Button>
        </VStack>
        {membros.length === 0 ? (renderNoMembers()) : (<ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{
      ...
      <ModalMembros showModal={isModalVisible} setShowModal={handleCloseModal} editMembro={(membroEditado) => {
            const novosMembros = membros.map((membro) => membro.email === membroEditado.email ? membroEditado : membro);
            setMembros(novosMembros);
        }} membroData={isEditMode && membroToEdit ? membroToEdit : undefined} addMembros={handleCadastrarMembroPress} setMembros={setMembros}/>

      <DeleteMembro showModal={isDeleteModalVisible} setShowModal={handleCloseDeleteModal} email={membroEmailToDelete} setMembros={setMembros}/>
    </Box>);
};
