...
const signUpSchema = z.object({
    password: z
        ...
        .regex(new RegExp(".*[A-Z].*"), "Deve conter pelo menos uma letra maiúscula")
        .regex(new RegExp(".*[a-z].*"), "Deve conter pelo menos uma letra minúscula")
        .regex(new RegExp(".*\\d.*"), "Deve conter pelo menos um número")
        .regex(new RegExp(".*[`~<>?,./!@#$%^&*()\\-_+=\"'|{}\\[\\];:\\\\].*"), "Deve conter pelo menos um caractere especial"),
    confirmpassword: z
        .string()
        .min(8, "A senha deve ter no mínimo 8 caracteres")
        .regex(new RegExp(".*[A-Z].*"), "Deve conter pelo menos uma letra maiúscula")
        ...
        if (data.password === data.confirmpassword) {
            ...
            if (response.success) {
                toast.show({
                    ...
                    render: ({ id }) => (<Toast nativeID={id} action="success">
              <ToastTitle>Usuário cadastrado com sucesso!</ToastTitle>
                ...
                toast.show({
                    placement: "bottom right",
                    render: ({ id }) => (<Toast nativeID={id} action="error">
      ...
      <VStack className="w-full">
        <VStack space="xl" className="w-full">
          <FormControl isInvalid={!!errors.password}>
            ...
            validate: async (value) => {
                try {
                  ...
                  <InputField className="text-sm" placeholder="Senha" value={value} onChangeText={onChange} onBlur={onBlur} onSubmitEditing={handleKeyPress} enterKeyHint="done" type={showPassword ? "text" : "password"}/>
          ...
          <FormControl isInvalid={!!errors.confirmpassword}>
            ...
            validate: async (value) => {
                try {
                  ...
                  <InputField placeholder="Confirmar Senha" className="text-sm" value={value} onChangeText={onChange} onBlur={onBlur} onSubmitEditing={handleKeyPress} enterKeyHint="done" type={showConfirmPassword ? "text" : "password"}/>
          ...
          <FormControl isInvalid={!!errors.curso}>
            ...
            <Controller name="curso" control={control} render={({ field: { onChange, value } }) => (<Select selectedValue={value} onValueChange={onChange}>
                  ...
                  <SelectPortal>
                    ...
                    <SelectContent>
                      ...
                      {cursos.map((curso) => (<SelectItem key={curso.id} value={String(curso.id)} label={curso.nome}/>))}
                    </SelectContent>
                  </SelectPortal>
                </Select>)}/>
            <FormControlError>
          ...
          <FormControl isInvalid={!!errors.telefone}>
            ...
            <Controller name="telefone" control={control} render={({ field: { onChange, value } }) => (<Input>
                  <InputField className="text-sm" placeholder="(xx) xxxxx-xxxx" value={value} keyboardType="phone-pad" onChangeText={(text) => {
          ...
          <FormControl isInvalid={!!errors.dataNascimento}>
            ...
            <Controller name="dataNascimento" control={control} render={({ field: { onChange, value } }) => (<Input>
                  <InputField className="text-sm" placeholder="dd/mm/aaaa" value={value} keyboardType="numeric" onChangeText={(text) => {
                const formatted = text
                    .replace(/\D/g, "")
