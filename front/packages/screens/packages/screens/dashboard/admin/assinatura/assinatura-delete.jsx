...
export const deletePlano = ({ showModal, setShowModal, assinnaturaId, updateAssianturaList, }) => {
    const handleDelete = async () => {
        try {
            const response = await deleteAssinatura(assinnaturaId);
            if (response.success) {
                updateAssianturaList();
                setShowModal(false);
            }
        }
        catch (error) {
          ...
          <ModalBody className="px-10 py-6">
            ...
            <VStack space="2xl">
              <HStack className="items-center justify-between">
                <Button onPress={() => setShowModal(false)} className="flex-1 mr-2 bg-gray-200">
