...
const MainContent = () => {
    ...
    const renderNoItems = () => (<NoItemsFound message="Nenhum plano encontrado."/>);
    const renderItems = () => (<Grid className="gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      {items.map((item, index) => (<GridItem key={index} _extra={{
                className: "flex-1 p-4 relativ",
            }}>
          <Pressable className="w-full">
            <Box className="overflow-hidden rounded-md h-72">
              <Image source={item.src} alt={item.title} height={"100%"} width={"100%"}/>
            </Box>
            <Box className="mt-2">
    ...
    return (<Box className="flex-1">
      <VStack className="p-4 pb-0 md:px-10 md:pt-6 w-full" space="2xl">
        ...
        {items.length === 0 ? (renderNoItems()) : (<ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1 }} className="p-4">
            {renderItems()}
          </ScrollView>)}
      </VStack>
    </Box>);
