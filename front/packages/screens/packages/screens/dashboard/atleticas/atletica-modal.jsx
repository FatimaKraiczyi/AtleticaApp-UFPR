...
export const ModalAtletica = ({ showModal, setShowModal, addAtletica, editAtletica, atleticaData, }) => {
    ...
    const [cursoFields, setCursoFields] = useState(["cursoIds"]);
    ...
    useEffect(() => {
        const fetchCursos = async () => {
            ...
            if (response.success) {
                ...
                const uniqueDepartamentos = Array.from(new Set(response.data.map((curso) => curso.departamento)));
    ...
    useEffect(() => {
        if (departamentoSelecionado) {
            const filtrados = cursos.filter((curso) => curso.departamento === departamentoSelecionado);
        ...
        try {
            if (atleticaData) {
                if (atleticaData.id !== undefined) {
                    const response = await updateAtletica(atleticaData.id, atleticaPayload);
                    if (response.success) {
      ...
      <ModalContent>
        ...
        <ModalBody className="px-10 py-6 max-h-[70vh] overflow-y-auto">
          <Center className="w-full mb-6">
            ...
            ? { uri: profileImage }
            : require("@/shared/assets/dashboard/dashboard-layout/image2.png")} alt={"Imagem de perfil"}/>
          ...
          <VStack space="xl">
            <FormControl isInvalid={!!errors.nome}>
              ...
              <Controller defaultValue="" name="nome" control={control} render={({ field: { onChange, onBlur, value } }) => (<Input>
                    <InputField placeholder="Nome da Atlética" type="text" value={value} onChangeText={onChange} onBlur={onBlur} onSubmitEditing={handleKeyPress} returnKeyType="done"/>
                  </Input>)}/>
              <FormControlError>
              ...
              <Controller defaultValue="" name="descricao" control={control} render={({ field: { onChange, onBlur, value } }) => (<Input>
                    <InputField placeholder="Descrição" type="text" value={value} onChangeText={onChange} onBlur={onBlur} onSubmitEditing={handleKeyPress} returnKeyType="done"/>
                  </Input>)}/>
              <FormControlError>
                <FormControlErrorIcon size="md" as={AlertTriangle}/>
                <FormControlErrorText>
              ...
              <Controller defaultValue="" name="departamento" control={control} render={({ field: { onChange } }) => (<Select onValueChange={(value) => {
                onChange(value);
                setDepartamentoSelecionado(value);
                    ...
                    <SelectPortal>
                      ...
                      <SelectContent>
                        ...
                        {departamentos.map((departamento) => (<SelectItem key={departamento} value={departamento} label={departamento}>
                            {departamento}
                          </SelectItem>))}
                      </SelectContent>
                    </SelectPortal>
            ...
            {cursoFields.map((field, index) => (<FormControl key={field} isInvalid={!!errors.cursoIds}>
                <FormControlLabel className="mb-2 flex items-center">
                  <FormControlLabelText>Curso</FormControlLabelText>
                </FormControlLabel>
                <div className="flex items-center w-full">
                  <Controller defaultValue="" name={`cursoIds.${index}`} control={control} render={({ field: { onChange } }) => (<Select onValueChange={onChange} isDisabled={!departamentoSelecionado} className="flex-1">
                        <SelectTrigger variant="outline" size="md">
                        ...
                        <SelectPortal>
                          ...
                          <SelectContent>
                            ...
                            {cursosFiltrados.map((curso) => (<SelectItem key={curso.id} value={String(curso.id)} label={curso.nome}>
                                {curso.nome}
                              </SelectItem>))}
                          </SelectContent>
                        </SelectPortal>
                      </Select>)}/>
                  {index === cursoFields.length - 1 && (<Button onPress={addCursoField} className="ml-2 p-1" variant="outline" size="sm" disabled={!departamentoSelecionado}>
                      <Icon as={PlusIcon} size="sm"/>
                    </Button>)}
                  {index > 0 && (<Button onPress={() => removeCursoField(index)} className="ml-2 p-1" variant="outline" size="sm">
            ...
            <FormControl isInvalid={!!errors.atividades}>
              ...
              <Controller defaultValue="" name="atividades" control={control} render={({ field: { onChange, onBlur, value } }) => (<Input>
                    <InputField placeholder="Atividades esportivas" type="text" value={value} onChangeText={onChange} onBlur={onBlur} onSubmitEditing={handleKeyPress} returnKeyType="done"/>
                  </Input>)}/>
              <FormControlError>
                <FormControlErrorIcon size="md" as={AlertTriangle}/>
                <FormControlErrorText>
