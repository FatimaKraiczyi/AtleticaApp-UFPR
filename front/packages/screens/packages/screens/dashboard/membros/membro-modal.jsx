...
export const ModalMembros = ({ showModal, setShowModal, addMembros, editMembro, membroData, setMembros, }) => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            if (membroData) {
                const response = await editarMembro(membroData.email, membroPayload.administrador);
    ...
    return (<Modal isOpen={showModal} finalFocusRef={ref} size="lg">
      <ModalBackdrop />
      <ModalContent>
        ...
        <ModalBody className="px-10 py-6">
          <VStack space="xl">
            <FormControl isInvalid={!!errors.email}>
              ...
              <Controller name="email" defaultValue="" control={control} render={({ field: { onChange, onBlur, value } }) => (<Input>
                    <InputField placeholder="Email" type="text" value={value} onChangeText={onChange} onBlur={onBlur} onSubmitEditing={handleKeyPress} enterKeyHint="done"/>
                  </Input>)}/>
              <FormControlError>
            ...
            <FormControl isInvalid={!!errors.nomeAtletica}>
              ...
              <Controller name="nomeAtletica" defaultValue="" control={control} render={({ field: { onChange, onBlur, value } }) => (<Input>
                    <InputField placeholder="Nome da Atlética" type="text" value={value} onChangeText={onChange} onBlur={onBlur} onSubmitEditing={handleKeyPress} enterKeyHint="done"/>
                  </Input>)}/>
              <FormControlError>
                <FormControlErrorIcon as={AlertTriangle} size="md"/>
                <FormControlErrorText>
            ...
            <FormControl>
              ...
              <Controller name="administrador" control={control} defaultValue={false} render={({ field: { onChange, value } }) => (<Switch value={value} onValueChange={(value) => {
                onChange(value);
                setIsAdmin(value);
            }}/>)}/>
            </FormControl>
